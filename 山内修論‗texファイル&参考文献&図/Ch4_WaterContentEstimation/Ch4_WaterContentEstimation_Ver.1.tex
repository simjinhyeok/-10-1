...
減少する．
% 従って，含水比の変動に伴い土の分光反射率スペクトルも変動する．
そこで，これを利用することによって，スペクトル画像から含水比を推定する．% "用いて"の後は"から～"
% 水は近赤外の光を吸収するため，土に含まれる水の量によって土の分光反射率スペクトルが変動することを述べる．
% そして，含水比を推定するために分光反射率スペクトルを用いることを述べる．
...
最後に，その差を用いて含水比を推定する．
% マルチスペクトル画像から本研究で使用する2つの波長帯を取得する様子を図\ref{fig:spectral_image_extraction}に示す．
% 図\ref{fig:spectral_image_extraction}に示すように，
% 撮影したスペクトル画像から水が光を吸収する波長帯と水が光を吸収しない波長帯を選択して，
% それぞれにおいて分光反射率を計算し，その差を用いて含水比を推定する．

% 図\ref{fig:spectral_image_extraction}において上側のマルチスペクトル画像から取得された
% 水が光を吸収しない波長帯と水が光を吸収する近赤外の波長帯においてそれぞれ分光反射率を計算し，
% その差を用いて含水比を推定する．
