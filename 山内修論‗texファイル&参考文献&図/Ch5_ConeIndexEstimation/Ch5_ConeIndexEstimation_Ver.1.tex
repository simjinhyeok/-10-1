...
% 第\ref{ch:SoilTypeDiscrimination}章で識別した
土の種類と
% 第\ref{ch:WaterContentEstimation}章で推定した
含水比を用いた
コーン指数推定の詳細について述べる．
...
\begin{figure}[b]
      \begin{center}
            \hspace{1.5cm}\includegraphics[width=13.5cm]{./Ch5_ConeIndexEstimation/Fig/spectralcamera_arrangement_Ver_2_compressed.pdf} % 画像が左に寄りすぎないように調整
            \caption{スペクトルカメラの配置}
...
% 含水比の推定が可能なマルチスペクトル画像を用いて
900 $\sim$ 1700nmの波長帯の分光反射率と
水が光を吸収しない570nmの波長帯の分光反射率を取得し，
その2つの分光反射率の差から含水比を推定した．
% 含水比の推定に関して，より詳細に述べる？（フィッティングについての説明と図を追加）

% \ref{ssec:ConeindexEstimationExperimentSetting}項でも述べた通り，ハイパースペクトル画像とマルチスペクトル画像は一般的なRGB画像と比較して入射光を分光させる波長帯の数が多いので，
% 一般的なRGB画像よりも強い光が必要になる．
...
% \subsection{不適切なハイパースペクトル画像とマルチスペクトル画像の除外}
...
マルチスペクトル画像については，画像自体の明るさと，分光反射率スペクトルが既知の校正シートの輝度値の分散の程度を確認した．
